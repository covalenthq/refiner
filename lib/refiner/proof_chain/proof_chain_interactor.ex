defmodule Refiner.ProofChain.Interactor do
  require Logger
  ...
  defp get_bsp_proofchain() do
    ...
    {:ok, proofchain} = Refiner.RPC.PublicKeyHash.parse(proofchain_address)
    proofchain
  end

  defp get_brp_proofchain() do
    proofchain_address = Application.get_env(:refiner, :brp_proofchain_address)
    {:ok, proofchain} = Refiner.RPC.PublicKeyHash.parse(proofchain_address)
    proofchain
  end
  ...
  defp make_call(data, proofchain) do
    ...
    with {:ok, res} <- Refiner.Network.EthereumMainnet.eth_call(tx) do
      res
    end
  end
  ...
  def is_block_result_session_open(block_height) do
    ...
    chain_id = Application.get_env(:refiner, :block_specimen_chain_id)
  ...
  defp get_operator_wallet() do
    operator_private_key = Application.get_env(:refiner, :operator_private_key)
    Refiner.Wallet.load(Base.decode16!(operator_private_key, case: :lower))
  end
  ...
  defp send_eip1559_signed_tx(
    ...
    {:ok, block} = Refiner.Network.EthereumMainnet.eth_getBlockByNumber(:latest)
    ...
    tx = %Refiner.RPC.EthereumClient.TransactionEIP1559{
      type: 2,
      nonce: nonce,
      to: to,
    ...
    Refiner.RPC.EthereumClient.TransactionEIP1559.signed_by(tx, sender)
  end
  ...
  defp get_eip1559_signed_tx(sender, nonce, to, estimated_gas_limit, data, proofchain_chain_id) do
    case proofchain_chain_id do
      ...
      31_337 ->
        {:ok, fee_history} = Refiner.Network.EthereumMainnet.eth_feeHistory()
      ...
      _ ->
        {:ok, max_priority_fee_per_gas_hex} =
          Refiner.Network.EthereumMainnet.eth_maxPriorityFeePerGas()
  ...
  defp get_legacy_signed_tx(sender, nonce, to, estimated_gas_limit, data, proofchain_chain_id) do
    gas_price = Refiner.Network.EthereumMainnet.eth_gasPrice!()

    tx = %Refiner.RPC.EthereumClient.Transaction{
      nonce: nonce,
      gas_price: gas_price,
      gas_limit: estimated_gas_limit,
      to: to,
    ...
    Refiner.RPC.EthereumClient.Transaction.signed_by(tx, sender)
  end
  ...
  def submit_block_result_proof(
    ...
    {:ok, recent_gas_limit} = Refiner.Network.EthereumMainnet.gas_limit(:latest)

    try do
      estimated_gas_limit =
        Refiner.Network.EthereumMainnet.eth_estimateGas!(
          from: sender.address,
          to: to,
          data: data,
      ...
      nonce = Refiner.Network.EthereumMainnet.next_nonce(sender.address)
      proofchain_chain_id = Application.get_env(:refiner, :proofchain_chain_id)

      signed_tx =
        get_eip1559_signed_tx(sender, nonce, to, estimated_gas_limit, data, proofchain_chain_id)

      with {:ok, txid} <- Refiner.Network.EthereumMainnet.eth_sendTransaction(signed_tx) do
        :ok = Events.brp_proof(System.monotonic_time(:millisecond) - start_proof_ms)
        Logger.info("#{block_height} txid is #{txid}")
        {:ok, :submitted}
      end
    rescue
      e in Refiner.RPCError ->
        cond do
