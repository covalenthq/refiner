defmodule Refiner.Journal do
  ...
  def items_with_status(status) do
    if status in [:commit, :abort, :discover, :skip] do
      {:ok, items} = GenServer.call(Refiner.Journal, {:workitem, :fetch, status}, :infinity)
      items
    else
  ...
  def last_started_block() do
    GenServer.call(Refiner.Journal, {:blockh, :fetch})
  end
  ...
  def discover(id) do
    GenServer.call(Refiner.Journal, {:workitem, :discover, id}, 500_000)
  end

  @spec commit(any) :: any
  def commit(id) do
    GenServer.call(Refiner.Journal, {:workitem, :commit, id}, 500_000)
  end

  @spec abort(any) :: any
  def abort(id) do
    GenServer.call(Refiner.Journal, {:workitem, :abort, id}, 500_000)
  end

  @spec skip(any) :: any
  def skip(id) do
    GenServer.call(Refiner.Journal, {:workitem, :skip, id}, 500_000)
  end

  # moonbeam block_height status logging APIs
  @spec block_height_started(any) :: any
  def block_height_started(height) do
    GenServer.call(Refiner.Journal, {:blockh, :start, height}, 500_000)
  end

  @spec block_height_committed(any) :: any
  def block_height_committed(height) do
    GenServer.call(Refiner.Journal, {:blockh, :commit, height}, 500_000)
  end
end
