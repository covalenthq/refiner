defmodule Refiner.Avro.BlockSpecimen do
  ...
  def decode_dir(dir_path) do
    Refiner.Util.get_file_paths(dir_path)
    |> Enum.map(fn file ->
      [file]
      |> Stream.map(&Refiner.Avro.BlockSpecimen.decode_file/1)
    end)
    |> List.flatten()
    |> Enum.sort()
  end
  ...
  def encode_dir(dir_path) do
    ...
    |> Enum.map(fn file ->
      [file]
      |> Stream.map(&Refiner.Avro.BlockSpecimen.encode_file/1)
    end)
    |> List.flatten()
    |> Enum.sort()
  end
