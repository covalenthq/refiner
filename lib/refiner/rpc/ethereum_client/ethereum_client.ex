defmodule Refiner.RPC.EthereumClient do
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [used_with_opts: opts] do
      ...
      def call(rpc_method, rpc_params, decode_fun \\ nil) do
        ...
        resp = client() |> Refiner.RPC.JSONRPC.Client.call(rpc_method, rpc_params, default_opts)
      ...
      defmacrop unwrap!(rpc_resp) do
        quote do
          case unquote(rpc_resp) do
            {:ok, value} -> value
            error -> raise Refiner.RPCError, error
          end
        end
      end
      ...
      def next_nonce(%Refiner.RPC.PublicKeyHash{} = pkh) do
