defmodule Refiner.BlockProcessor do
  use GenServer
  require Logger
  ...
  def sync_queue(%Refiner.BlockSpecimen{} = block_specimen) do
    Logger.info("submitting #{block_specimen.block_height} to evm http server...")

    start_execute_ms = System.monotonic_time(:millisecond)

    case GenServer.call(Refiner.BlockProcessor, {:process, block_specimen.contents}, 60_000) do
      {:ok, block_result} ->
